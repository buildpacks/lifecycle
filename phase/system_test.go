...
func TestMergeSystemBuildpacks(t *testing.T) {
	...
	t.Run("skips system buildpacks that are already in the group", func(t *testing.T) {
		order := buildpack.Order{
			{Group: []buildpack.GroupElement{
				{ID: "bp-1", Version: "1.0.0"},
				{ID: "bp-2", Version: "2.0.0"},
				{ID: "bp-3", Version: "3.0.0"},
			}},
		}
		...
		system := files.System{
			Pre: files.SystemBuildpacks{
				Buildpacks: []files.SystemBuildpack{
					{ID: "bp-3", Version: "3.0.0"}, // duplicate
					{ID: "pre-bp", Version: "0.5.0"},
				},
			},
			Post: files.SystemBuildpacks{
				Buildpacks: []files.SystemBuildpack{
					{ID: "bp-1", Version: "1.0.0"}, // duplicate
					{ID: "post-bp", Version: "5.0.0"},
				},
			},
		}
		...
		h.AssertEq(t, len(merged), 1)
		// Should have: pre-bp, bp-1, bp-2, bp-3, post-bp (5 total, not 7)
		h.AssertEq(t, len(merged[0].Group), 5)
		h.AssertEq(t, merged[0].Group[0].ID, "pre-bp")  // only non-duplicate pre
		h.AssertEq(t, merged[0].Group[1].ID, "bp-1")    // from order
		h.AssertEq(t, merged[0].Group[2].ID, "bp-2")    // from order
		h.AssertEq(t, merged[0].Group[3].ID, "bp-3")    // from order
		h.AssertEq(t, merged[0].Group[4].ID, "post-bp") // only non-duplicate post
	})

	t.Run("skips system buildpacks with same ID regardless of version", func(t *testing.T) {
		order := buildpack.Order{
			{Group: []buildpack.GroupElement{
				{ID: "bp-1", Version: "1.0.0"},
			}},
		...
		system := files.System{
			Pre: files.SystemBuildpacks{
				Buildpacks: []files.SystemBuildpack{
					{ID: "bp-1", Version: "2.0.0"}, // same ID, different version - should be skipped
				},
			},
		}
		...
		h.AssertEq(t, len(merged), 1)
		h.AssertEq(t, len(merged[0].Group), 1) // only the order version
		h.AssertEq(t, merged[0].Group[0].ID, "bp-1")
		h.AssertEq(t, merged[0].Group[0].Version, "1.0.0") // order version wins
	})
}

func TestFilterDuplicates(t *testing.T) {
	t.Run("filters out exact duplicates", func(t *testing.T) {
		systemBps := []buildpack.GroupElement{
			{ID: "bp-1", Version: "1.0.0"},
			{ID: "bp-2", Version: "2.0.0"},
			{ID: "bp-3", Version: "3.0.0"},
		}
		existingGroup := []buildpack.GroupElement{
			{ID: "bp-2", Version: "2.0.0"},
		}

		filtered := filterDuplicates(systemBps, existingGroup)

		h.AssertEq(t, len(filtered), 2)
		h.AssertEq(t, filtered[0].ID, "bp-1")
		h.AssertEq(t, filtered[1].ID, "bp-3")
	})

	t.Run("filters by ID only regardless of version", func(t *testing.T) {
		systemBps := []buildpack.GroupElement{
			{ID: "bp-1", Version: "2.0.0"},
		}
		existingGroup := []buildpack.GroupElement{
			{ID: "bp-1", Version: "1.0.0"},
		}
		...
		h.AssertEq(t, len(filtered), 0) // filtered out because ID matches
	})

	t.Run("returns all when no duplicates", func(t *testing.T) {
		systemBps := []buildpack.GroupElement{
			{ID: "bp-1", Version: "1.0.0"},
			{ID: "bp-2", Version: "2.0.0"},
		}
	...
	t.Run("returns empty when all are duplicates", func(t *testing.T) {
		systemBps := []buildpack.GroupElement{
			{ID: "bp-1", Version: "1.0.0"},
			{ID: "bp-2", Version: "2.0.0"},
		}
