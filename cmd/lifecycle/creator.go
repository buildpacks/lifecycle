...
func (c *createCmd) Exec() error {
	...
	if c.PlatformAPI.AtLeast("0.7") {
		...
		detector, err := detectorFactory.NewDetector(c.AppDir, c.BuildConfigDir, c.OrderPath, c.PlatformDir, c.AnalyzedPath, cmd.DefaultLogger)
		if err != nil {
		...
		detector, err := detectorFactory.NewDetector(c.AnalyzedPath, c.AppDir, c.BuildConfigDir, c.OrderPath, c.PlatformDir, cmd.DefaultLogger)
		if err != nil {
			return unwrapErrorFailWithMessage(err, "initialize detector")
		}
		group, plan, err = doDetect(detector, c.Platform)
