   ...
   "process-types": {
      "sha": "process-types-digest"
			...
			when("the launch flag is in the top level table", func() {
				it.Before(func() {
					exporter.Buildpacks = []buildpack.GroupBuildable{{ID: "bad.buildpack.id", API: api.Buildpack.Latest().String()}}
...
`
						...
						h.AssertEq(t, report.Build.BOM, []buildpack.BOMEntry{
							{
								...
								Buildpack: buildpack.GroupBuildable{ID: "buildpack.id", Version: "1.2.3"},
							},
							{
								Require: buildpack.Require{
									Name:     "dep2",
								...
								Buildpack: buildpack.GroupBuildable{ID: "other.buildpack.id", Version: "4.5.6"},
							},
						})
					})
				})
		...
		when("buildpack requires an escaped id", func() {
			it.Before(func() {
				exporter.Buildpacks = []buildpack.GroupBuildable{{ID: "some/escaped/bp/id", API: api.Buildpack.Latest().String()}}
		...
		when("buildpack API < 0.6", func() {
			it.Before(func() {
				exporter.Buildpacks = []buildpack.GroupBuildable{{ID: "old.buildpack.id", API: "0.5"}}
			})
