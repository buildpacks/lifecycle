...
import (
	...
	launchenv "github.com/buildpacks/lifecycle/platform/launch"
)
...
func (e *Exporter) setEnv(opts ExportOptions, launchMD launch.Metadata) error {
	...
	e.Logger.Debugf("Setting %s=%s", guard.EnvDeprecationMode, guard.ModeQuiet)
	if err := opts.WorkingImage.SetEnv(guard.EnvDeprecationMode, guard.ModeQuiet); err != nil {
		return errors.Wrapf(err, "set app image env %s", platform.EnvAppDir)
	}
	...
	if e.supportsMulticallLauncher() {
		...
		e.Logger.Debugf("Setting %s=%s", launchenv.EnvProcessType, opts.DefaultProcessType)
		if err := opts.WorkingImage.SetEnv(launchenv.EnvProcessType, opts.DefaultProcessType); err != nil {
			return errors.Wrapf(err, "set app image env %s", launchenv.EnvProcessType)
		}
	}
	return nil
}
