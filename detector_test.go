...
func testDetector(t *testing.T, when spec.G, it spec.S) {
	when("#Detect", func() {
		...
		it("should select the first passing group", func() {
			...
			if !hasEntries(plan.Entries, []dataformat.BuildPlanEntry(nil)) {
		...
		it("should convert top level versions to metadata versions", func() {
			...
			resolver.EXPECT().Resolve(group, detector.Runs).Return(group, []dataformat.BuildPlanEntry{
				{
			...
			if !hasEntries(plan.Entries, []dataformat.BuildPlanEntry{
				{
					Providers: []buildpack.GroupBuildpack{
						{ID: "A", Version: "v1"},
					},
		...
		it("should update detect runs for each buildpack", func() {
			...
			resolver.EXPECT().Resolve(group, detector.Runs).Return(group, []dataformat.BuildPlanEntry{}, nil)
	...
	when("#Resolve", func() {
		...
		it("should return a build plan with matched dependencies", func() {
			...
			if !hasEntries(entries, []dataformat.BuildPlanEntry{
				{
					Providers: []buildpack.GroupBuildpack{
						{ID: "A", Version: "v1"},
						{ID: "C", Version: "v2"},
...
func hasEntry(l []dataformat.BuildPlanEntry, entry dataformat.BuildPlanEntry) bool {
	for _, e := range l {
...
func hasEntries(a, b []dataformat.BuildPlanEntry) bool {
	if len(a) != len(b) {
		return false
	}
	for _, e := range a {
