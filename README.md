...
`LIFECYCLE_VERSION` defaults to the value returned by `git describe --tags` if not on a release branch. It can be changed by prepending `LIFECYCLE_VERSION=<some version>` to the
