...
func TestLayerAnalyzer(t *testing.T) {
...
func testLayerAnalyzer(t *testing.T, when spec.G, it spec.S) {
	var (
		layerDir       string
		layerAnalyzer  lifecycle.LayerAnalyzer
	...
	when("#Analyze", func() {
		it.Before(func() {
			buildpacks = []buildpack.GroupBuildpack{{ID: "metadata.buildpack"}, {ID: "no.cache.buildpack"}, {ID: "escaped/buildpack/id"}}
		})
		...
		when("only app metadata is present", func() {
			it.Before(func() {
				layerMetaDataJSON := h.MustReadFile(t, filepath.Join("testdata", "restorer", "app_metadata.json"))
		...
		when("only cache metadata is present", func() {
			it.Before(func() {
				cacheMetaDataJSON := h.MustReadFile(t, filepath.Join("testdata", "restorer", "cache_metadata.json"))
		...
		when("app and cache metadata are present", func() {
			...
			when("subset of buildpacks are detected", func() {
				it.Before(func() {
					buildpacks = []buildpack.GroupBuildpack{{ID: "no.cache.buildpack"}}
				})
