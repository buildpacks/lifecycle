...
func testLayerMetadataRestorer(t *testing.T, when spec.G, it spec.S) {
	var (
		...
		layerSHAStore         layermetadata.LayerSHAStore
	...
	it.Before(func() {
		...
		layerMetadataRestorer = layermetadata.NewLayerMetadataRestorer(&logger, layerDir, skipLayers)
